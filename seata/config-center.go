...
func GetConfigCenterConfigurations(key string) {
	v := url.Values{}
	if key != "" {
		v.Add("key", key)
	}

	url := HTTPProtocol + GetAuth().GetAddress() + ConfigCenterURL
...
func SetConfigCenterConfiguration(key string, value string) {
